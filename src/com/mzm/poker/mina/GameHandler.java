...
import com.mzm.poker.protobuf.CoreProtocol.MessagePack;
 ...
 * 
 */
public class GameHandler extends IoHandlerAdapter implements
...
//		System.out.println("toString : " + messagePack.getContent().toString());
		...
		Method m = executor.getClass().getMethod(methodName, IoSession.class,
				MessageContent.class);
		if (m == null) {
			logger.error("method is null. methodName: " + methodName
					+ ". logicName: " + logicName);
			return;
		}
		...
		if (diff > 1000) {
			logger.debug("spending too much time on logicName : [" + logicName + "] methodName : [" + methodName +"]");
		}
		...
		logger.debug("sent message pack : " + returnBuilder.toString());
	...
	public void exceptionCaught(IoSession session, Throwable cause)
		...
		try {
			...
			//
			// }
			logger.error("mina exception caught : " + cause.getMessage());
		} catch (Exception e) {
			logger.error(e, e);
		}
	}
